...
class Evidence(Base):
    ...
    __table_args__ = (UniqueConstraint("control_id", "source_system", "collected_at", name="uq_evidence_snapshot"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
...
class Evaluation(Base):
    ...
    __table_args__ = (UniqueConstraint("control_id", "evaluated_at", name="uq_evaluation_point_in_time"),)
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=uuid_str)
    control_id: Mapped[str] = mapped_column(String(32), index=True)
